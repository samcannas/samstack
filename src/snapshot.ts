...
import { ensurePathWithinRoots, getDefaultArtifactPath, getSafeRoots } from './platform';
...
export const SNAPSHOT_FLAGS: Array<{
  ...
  { short: '-o', long: '--output', description: 'Output path for annotated screenshot (default: temp dir + samstack-annotated.png)', takesValue: true, valueHint: '<path>', optionKey: 'outputPath' },
...
export async function handleSnapshot(
  ...
  if (opts.annotate) {
    const screenshotPath = opts.outputPath || getDefaultArtifactPath('samstack-annotated.png');
    // Validate output path (consistent with screenshot/pdf/responsive)
    const resolvedPath = require('path').resolve(screenshotPath);
    ensurePathWithinRoots(resolvedPath, getSafeRoots());
    try {
