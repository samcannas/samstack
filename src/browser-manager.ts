...
import { COMMAND_NAME, STATE_DIR_NAME } from './platform';
...
export class BrowserManager {
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      console.error(`[${COMMAND_NAME}] FATAL: Chromium process crashed or was killed. Server exiting.`);
      console.error(`[${COMMAND_NAME}] Console/network logs flushed to ${STATE_DIR_NAME}/samstack-*.log`);
      process.exit(1);
    });
  ...
  getPage(): Page {
    const page = this.pages.get(this.activeTabId);
    if (!page) throw new Error(`No active page. Use "${COMMAND_NAME} goto <url>" first.`);
    return page;
  }
