...
import { COMMAND_NAME, ensurePathWithinRoots, getSafeRoots } from './platform';
...
export async function handleReadCommand(
  ...
  switch (command) {
    ...
    case 'js': {
      const expr = args[0];
      if (!expr) throw new Error(`Usage: ${COMMAND_NAME} js <expression>`);
    ...
    case 'eval': {
      ...
      if (!filePath) throw new Error(`Usage: ${COMMAND_NAME} eval <js-file>`);
    ...
    case 'css': {
      ...
      if (!selector || !property) throw new Error(`Usage: ${COMMAND_NAME} css <selector> <property>`);
      const resolved = bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'attrs': {
      const selector = args[0];
      if (!selector) throw new Error(`Usage: ${COMMAND_NAME} attrs <selector>`);
      const resolved = bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'is': {
      ...
      if (!property || !selector) throw new Error(`Usage: ${COMMAND_NAME} is <property> <selector>\nProperties: visible, hidden, enabled, disabled, checked, editable, focused`);
