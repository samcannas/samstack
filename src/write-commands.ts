...
import { COMMAND_NAME, ensurePathWithinRoots, getLocalUrl, getSafeRoots, openUrl } from './platform';
...
export async function handleWriteCommand(
  ...
  switch (command) {
    case 'goto': {
      const url = args[0];
      if (!url) throw new Error(`Usage: ${COMMAND_NAME} goto <url>`);
    ...
    case 'click': {
      ...
      if (!selector) throw new Error(`Usage: ${COMMAND_NAME} click <selector>`);
    ...
    case 'fill': {
      ...
      if (!selector || !value) throw new Error(`Usage: ${COMMAND_NAME} fill <selector> <value>`);
      const resolved = bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'select': {
      const [selector, ...valueParts] = args;
      const value = valueParts.join(' ');
      if (!selector || !value) throw new Error(`Usage: ${COMMAND_NAME} select <selector> <value>`);
      const resolved = bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'hover': {
      const selector = args[0];
      if (!selector) throw new Error(`Usage: ${COMMAND_NAME} hover <selector>`);
      const resolved = bm.resolveRef(selector);
      if ('locator' in resolved) {
    ...
    case 'type': {
      ...
      if (!text) throw new Error(`Usage: ${COMMAND_NAME} type <text>`);
    ...
    case 'press': {
      const key = args[0];
      if (!key) throw new Error(`Usage: ${COMMAND_NAME} press <key> (e.g., Enter, Tab, Escape)`);
    ...
    case 'wait': {
      const selector = args[0];
      if (!selector) throw new Error(`Usage: ${COMMAND_NAME} wait <selector|--networkidle|--load|--domcontentloaded>`);
    ...
    case 'viewport': {
      const size = args[0];
      if (!size || !size.includes('x')) throw new Error(`Usage: ${COMMAND_NAME} viewport <WxH> (e.g., 375x812)`);
    ...
    case 'cookie': {
      ...
      if (!cookieStr || !cookieStr.includes('=')) throw new Error(`Usage: ${COMMAND_NAME} cookie <name>=<value>`);
    ...
    case 'header': {
      ...
      if (!headerStr || !headerStr.includes(':')) throw new Error(`Usage: ${COMMAND_NAME} header <name>:<value>`);
    ...
    case 'useragent': {
      const ua = args.join(' ');
      if (!ua) throw new Error(`Usage: ${COMMAND_NAME} useragent <string>`);
      bm.setUserAgent(ua);
    ...
    case 'upload': {
      ...
      if (!selector || filePaths.length === 0) throw new Error(`Usage: ${COMMAND_NAME} upload <selector> <file1> [file2...]`);
    ...
    case 'cookie-import': {
      ...
      if (!filePath) throw new Error(`Usage: ${COMMAND_NAME} cookie-import <json-file>`);
      // Path validation — prevent reading arbitrary files
      if (path.isAbsolute(filePath)) {
        ensurePathWithinRoots(filePath, getSafeRoots());
      }
    ...
    case 'cookie-import-browser': {
      ...
      const pickerUrl = getLocalUrl(port, '/cookie-picker');
      openUrl(pickerUrl);
