...
import { COMMAND_NAME, ensurePathWithinRoots, getDefaultArtifactPath, getSafeRoots } from './platform';

// Security: Path validation to prevent path traversal attacks
const SAFE_DIRECTORIES = getSafeRoots();

function validateOutputPath(filePath: string): void {
  ensurePathWithinRoots(filePath, SAFE_DIRECTORIES);
}
...
export async function handleMetaCommand(
  ...
  switch (command) {
    ...
    case 'tab': {
      ...
      if (isNaN(id)) throw new Error(`Usage: ${COMMAND_NAME} tab <id>`);
      bm.switchTab(id);
    ...
    case 'screenshot': {
      ...
      const page = bm.getPage();
      let outputPath = getDefaultArtifactPath('samstack-screenshot.png');
    ...
    case 'pdf': {
      const page = bm.getPage();
      const pdfPath = args[0] || getDefaultArtifactPath('samstack-page.pdf');
    ...
    case 'responsive': {
      const page = bm.getPage();
      const prefix = args[0] || getDefaultArtifactPath('samstack-responsive');
    ...
    case 'chain': {
      ...
      if (!jsonStr) throw new Error(`Usage: echo '[["goto","url"],["text"]]' | ${COMMAND_NAME} chain`);
    ...
    case 'diff': {
      ...
      if (!url1 || !url2) throw new Error(`Usage: ${COMMAND_NAME} diff <url1> <url2>`);
